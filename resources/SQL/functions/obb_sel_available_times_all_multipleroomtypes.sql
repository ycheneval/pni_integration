-- Function: salesforcetraining2.obb_sel_available_times(character, integer, character varying[])

-- DROP FUNCTION salesforcetraining2.obb_sel_available_times(character, integer, character varying[]);

CREATE OR REPLACE FUNCTION salesforcetraining2.obb_sel_available_times(IN event_id character, IN duration integer, IN room_type character varying[])
  RETURNS TABLE(day character varying, start_time timestamp without time zone, end_time timestamp without time zone) AS
$BODY$
DECLARE
/*
    Author: YCH
    History: V1.0  Dec 16 2015, Allowed to call with an aray of room types
...
$BODY$
  ...
  COST 1000
  ROWS 100;
ALTER FUNCTION salesforcetraining2.obb_sel_available_times(character, integer, character varying[])
